---
title: "How to prepare an *.md file for upload to gov.uk"
output: 
  html_document:
    keep_md: true
---
...
This template describes how to prepare a *.md file for upload to gov.uk. Guidance on md files for gov.uk can be found [here](https://www.gov.uk/guidance/how-to-publish-on-gov-uk/markdown).


## Instructions

On its own, this template will not be able to produce the file required by publications. Knitting this template will create a html file as well as an md file. Once the md file is produced, the user will need to run `convert_md()` on the md file.[^1] 
...
* 1-mortality statistics; (first image)
* 2-AMR statistics; (second image)
* 3-AMR by sex; (third image)
...
Email addresses format when surrounded by <>: <email@domain.co.uk>.
