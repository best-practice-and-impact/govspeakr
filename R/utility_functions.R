...
#'   original image name and corresponding govspeak reference
#' @param img_filenames character vector of files to be referenced in
#'   govspeak format (e.g. !!n). The filename must start and end with
#'   a number and have text in between (eg, "1-abcd-1.png")
#' @name generate_image_references
#' @title Generate govspeak image references
...
#' @param image_references dataframe of image file names and
#'   associated govspeak reference.
...
#' @title Convert markdown image references to govspeak
...
#' Remove R markdown multiline block elements (package warnings,
#'   but also multiline code blocks)
#' @param md_file string; markdown file text
#' @name remove_rmd_blocks
#' @title Remove R markdown multiline block elements (package
#'   warning and code block)
